...
test('Learning Experience API', async (t) => {
  ...
  const regRes = await app.inject({
    ...
    url: '/auth/register',
    payload: {
      email: `test-learn-${Date.now()}@example.com`,
      password: 'password',
      name: 'Tester',
    },
  });
  if (regRes.statusCode !== 201)
    throw new Error(`Register failed: ${regRes.payload}`);
  ...
  if (obRes.statusCode !== 200)
    throw new Error(`Onboarding failed: ${obRes.payload}`);
  ...
  await t.test(
    'POST /lessons/:userLessonId/parts/:partId/answer (Incorrect)',
    async () => {
      const res = await app.inject({
        method: 'POST',
      ...
      const progress = await app.db.query.userPartProgress.findFirst({
        where: (p, { and, eq }) =>
          and(eq(p.userLessonId, userLessonId), eq(p.partId, partId)),
      });
  ...
  await t.test(
    'POST /lessons/:userLessonId/parts/:partId/answer (Correct)',
    async () => {
      const res = await app.inject({
        method: 'POST',
        url: `/lessons/${userLessonId}/parts/${partId}/answer`,
  ...
  await t.test(
    'GET /lessons/generate/:jobId returns resultUserLessonId on DONE',
    async () => {
