...
test('expireLessons flags expired lessons and is idempotent', async (t) => {
  ...
  const expiredLesson = await db.insert(lessons).values({
  ...
  const freshLesson = await db.insert(lessons).values({
  ...
  const [checkExpired] = await db.select().from(lessons).where(eq(lessons.id, expiredLesson[0].id));
  assert.equal(checkExpired.isExpiredForReuse, true, 'Expired lesson should be flagged');

  const [checkFresh] = await db.select().from(lessons).where(eq(lessons.id, freshLesson[0].id));
  assert.equal(checkFresh.isExpiredForReuse, false, 'Fresh lesson should not be flagged');

  // Run expiry again to test idempotency
  const result2 = await expireLessons(db);
  assert.equal(result2.updated, 0, 'Should not update any lessons on second run');
