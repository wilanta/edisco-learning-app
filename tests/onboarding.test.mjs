...
test('Onboarding flow', async (t) => {
  ...
  await t.test(
    'PATCH /users/me/onboarding updates user preferences and sets credit',
    async () => {
      const res = await app.inject({
        method: 'PATCH',
  ...
  await t.test(
    'PATCH /users/me/onboarding is idempotent if already completed',
    async () => {
  ...
  await t.test(
    'PATCH /users/me/onboarding rejects invalid payload',
    async () => {
      const res = await app.inject({
        method: 'PATCH',
        url: '/users/me/onboarding',
