...
- **Delivery status (2026-09-19): COMPLETE for the latest user-authorized infrastructure-only scope.** Inspected clean Git state at `07871c2fe9578210aa97ac80834a1ed173f40f5b`, existing auth/onboarding/schema/worker/tests, and source specifications. No earlier Phase 4A implementation existed. Preserved working auth/onboarding and the existing GenerationJob schema; no new migration, provider abstraction, or service was introduced.
- **Scope adjustment:** The user's current instruction explicitly excludes generation frontend, actual lesson generation, embeddings/reuse, quota decrement, and expiry. It supersedes the broader original slice below for this delivery. No Track/Lesson/Part/UserLesson output is created. The opt-in placeholder completes jobs with `resultLessonId: null`, a temporary departure from API §4's eventual real-content success contract. New-track jobs retain `trackId: null`; continuation jobs retain their owned track ID. This is infrastructure verification, not a releasable generator or satisfaction of the original full G2 acceptance gate.
- **Implemented:** Authenticated `POST /lessons/generate` validates new/continuation input, ownership, onboarding, and positive quota without spending it; inserts PENDING; submits the UUID as BullMQ job ID; returns 202. Owner-scoped `GET /lessons/generate/:jobId` returns only contract fields. A separate worker reads authoritative SQL input, persists PROCESSING then DONE or FAILED, updates timestamps, and leaves terminal jobs unchanged on redelivery. Enqueue failure returns safe 503/GENERATION_FAILED and marks the pending row FAILED. Processor failures and terminal BullMQ failure events write a sanitized error. Producer Redis requests fail promptly; the worker reconnects and uses BullMQ's default stalled-job mechanism, with one processing attempt and no provider retries.
- **Files:** `apps/api/src/modules/lessons/generation.routes.ts`, API registration/dependencies, `apps/worker/src/{index,generation}.ts`, worker dependencies, shared status DTO, root scripts/lockfile/environment example, `tests/generation.test.mjs`, README, docs plan pointer, and HANDOFF. Corrected existing test cleanup ordering/JWT setup and stale database expectations for already-committed migration 0002; no auth/onboarding production behavior was changed.
- **Verification actually executed:** `npm run test:generation` passed (7 tests including parent); `npm test` passed (20); `npm run test:database` passed (8). The test uses a fresh PostgreSQL database, unique Redis prefix, and separate worker process; a database barrier proves observable PROCESSING without adding processor delays. Covered PENDING/queued ID, DONE, FAILED, ownership/input/onboarding/quota guards, continuation, duplicate enqueue/terminal redelivery, enqueue outage, and zero content/quota side effects. `npm run typecheck` passed; API/worker/database builds passed through test scripts. `npm run lint` passed with five pre-existing warnings outside new code. Changed-code formatting and `git diff --check` passed. Existing migrations were applied to the development database; no Phase 4A migration was needed. Ponytail review found no unnecessary abstractions to remove.
- **Limits before real providers:** No durable outbox/reconciliation or request idempotency yet; a crash between DB insert and queue submission, lost Redis data, or failed status persistence during DB outage can strand jobs. Terminal queue records are retained without a cleanup policy. These reliability decisions, concurrent admission, atomic content/assignment/quota commit, real result IDs, continuation curriculum/context, and input snapshots remain required before production generation. A continuing request without topic uses the track title only as placeholder input; it does not choose the next lesson. `GENERATION_PLACEHOLDER_ENABLED=true` must be explicitly set for this smoke path. No public fault-injection flag exists; failure tests corrupt only isolated fixtures.

The original broader plan below remains a record of deferred requirements, not a claim that the excluded work is complete:
