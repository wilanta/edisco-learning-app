...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Register the user
      const registerRes = await apiFetch<{ userId: string; token: string }>('/auth/register', {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col items-center min-h-screen p-6 max-w-sm mx-auto w-full pt-16">
      ...
      <div className="mt-6 text-sm text-center">
        Already have an account? <Link href="/login" className="text-blue-600 font-semibold underline">Log in</Link>
      </div>
    </div>
  );
}
