...
const tracksRoutes: FastifyPluginAsync = async (app) => {
  app.get('/', { preValidation: [app.authenticate] }, async (request, reply) => {
    ...
    const tracksList = await app.db
      ...
      .from(tracks)
      .leftJoin(userLessons, and(eq(tracks.id, userLessons.trackId), eq(userLessons.userId, userId)))
  ...
  app.get<{ Params: { trackId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!z.uuid().safeParse(trackId).success) {
        return reply.code(400).send({ error: 'VALIDATION_ERROR', message: 'Invalid track ID' });
      }
      ...
      const trackLessons = await app.db
        ...
        .where(and(eq(userLessons.trackId, trackId), eq(userLessons.userId, userId)))
